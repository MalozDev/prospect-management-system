...
import { clearToken, apiFetch, getStoredApiUser, setStoredApiUser } from "@/lib/api-client";
...
export default function SupervisorSettingsPage() {
  ...
  useEffect(() => {
    async function loadAvatarFromBackend() {
      ...
      if (stored.avatarUrl?.startsWith("data:")) return;
  ...
  return (
    <PageShell title="Supervisor Settings" description="Manage your basic profile and dashboard details.">
      <div className="grid gap-4">
            ...
            {!hasPhoto && (
              <div className="rounded-2xl bg-gray-50 border border-dashed border-gray-200 p-4 text-center">
                <div className="flex items-center justify-center gap-2 text-xs text-gray-500">
                  ...
                  <p>Your photo will appear on your profile across the app once uploaded.</p>
                </div>
              </div>
            )}
          </div>
        ...
        {/* ── Sales Targets ── */}
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
          <h3 className="font-semibold text-gray-900">Team Targets</h3>
          <p className="mt-1 text-sm text-gray-500">Set daily prospect capture and ODU sales targets for your team.</p>

          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {[
              { key: TARGET_KEYS.PROSPECT_DAILY, label: "Daily Prospects", desc: "Prospects per DSE per day" },
              { key: TARGET_KEYS.DAILY, label: "Daily Sales", desc: "ODU sales per DSE per day" },
              { key: TARGET_KEYS.WEEKLY, label: "Weekly Sales", desc: "ODU sales per DSE per week" },
              { key: TARGET_KEYS.MONTHLY, label: "Monthly Sales", desc: "ODU sales per DSE per month" },
              { key: TARGET_KEYS.TEAM, label: "Team Target", desc: "ODU sales for the entire team" },
            ].map(({ key, label, desc }) => (
              <label key={key} className="text-sm font-medium text-gray-700">
                <span className="mb-1 block">{label}</span>
                <span className="mb-1 block text-[10px] text-gray-400">{desc}</span>
                <input
                  type="number"
                  min="1"
                  value={targets[key] || ""}
